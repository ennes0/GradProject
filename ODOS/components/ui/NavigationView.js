...
const slopeAvgAbsPctToColor = (pct) => {
...
const chunkCoordsToMapFormat = (chunkCoords) => {
  if (!chunkCoords || !Array.isArray(chunkCoords)) return [];
  return chunkCoords
    .map((pt) => {
      if (Array.isArray(pt) && pt.length >= 2) {
        return { latitude: Number(pt[0]), longitude: Number(pt[1]) };
      }
...
const chunkAvgAbsSlopePct = (chunk) => {
...
export default function NavigationView({
  ...
  const currentInstruction = {
    ...
    text: nextInstruction.action === 'arrive'
      ? tx('Hedefinize ulaştınız', 'You have reached your destination')
      : `${fmtDistance(distanceToNextInstructionM)} ${tx('sonra', 'later')} ${getDirectionText(nextInstruction.action).toLowerCase()}`,
    street: secondInstruction ? `${tx('Ardından', 'Then')} ${getDirectionText(secondInstruction.action).toLowerCase()}` : '',
  };
  ...
  const navigationData = {
    ...
    estimatedTime: `${Math.round(etaMin)} ${tx('dk', 'min')}`,
  ...
  function getDirectionText(action) {
    switch (action) {
      case 'left': return tx('Sola Dön', 'Turn Left');
      case 'right': return tx('Sağa Dön', 'Turn Right');
      case 'straight': return tx('Düz Git', 'Go Straight');
      case 'uturn': return tx('Geri Dön', 'Make a U-turn');
      case 'arrive': return tx('Hedefe var', 'Arrive at destination');
      default: return tx('Devam Et', 'Continue');
    }
  }
    ...
    >
      <View style={styles.container}>
        ...
        >
          ...
          {/* Route Polyline (navigasyonda eğime göre renklendir) */}
          {routeCoords.length > 1 && (
            <>
              {Array.isArray(route?.slopePolylineChunks) && route.slopePolylineChunks.length > 0 ? (
                <>
                  {route.slopePolylineChunks.map((chunk, idx) => {
                    const pts = chunkCoordsToMapFormat(chunk?.coordinates);
                    if (pts.length < 2) return null;
                    const slopeColor = slopeAvgAbsPctToColor(chunkAvgAbsSlopePct(chunk));
                    return (
                      <React.Fragment key={`nav-slope-${idx}`}>
                        <Polyline
                          coordinates={pts}
        ...
        {!showFullMap && (
          ...
          >
            <View>
              <View style={styles.panelGrabberTouch} {...summaryPanResponder.panHandlers}>
              ...
              <View style={styles.panelHeaderRow}>
                <View style={styles.panelLiveBadge}>
                  ...
                  <Text style={styles.panelLiveText}>{tx('Canli navigasyon', 'Live navigation')}</Text>
                </View>
              ...
              >
                <View style={[styles.summarySlide, { width: summaryPagerWidth }]}>
                  <View style={styles.summarySlideHeader}>
                    <Text style={styles.summarySlideTitle}>{tx('Rota Ozeti', 'Route Summary')}</Text>
                  ...
                  <View style={styles.clusterGrid}>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Toplam', 'Total')}</Text>
                      <Text style={styles.clusterValue}>{navigationData.totalDistance}</Text>
                    </View>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Kalan Sure', 'Remaining Time')}</Text>
                      <Text style={styles.clusterValue}>{navigationData.estimatedTime}</Text>
                    </View>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Tirmanis', 'Climb')}</Text>
                      <Text style={styles.clusterValue}>{navigationData.totalClimb}</Text>
                    </View>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Ilerleme', 'Progress')}</Text>
                ...
                <View style={[styles.summarySlide, { width: summaryPagerWidth }]}>
                  <View style={styles.summarySlideHeader}>
                    <Text style={styles.summarySlideTitle}>{tx('Yukseklik Profili', 'Elevation Profile')}</Text>
                    <View style={styles.summarySlideBadge}>
                ...
                <View style={[styles.summarySlide, { width: summaryPagerWidth }]}>
                  <View style={styles.summarySlideHeader}>
                    <Text style={styles.summarySlideTitle}>{tx('Canli Durum', 'Live Status')}</Text>
                    <View style={styles.summarySlideBadge}>
                  ...
                  <View style={styles.clusterGrid}>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Hiz', 'Speed')}</Text>
                      <Text style={styles.clusterValue}>{navigationData.currentSpeed}</Text>
                    </View>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Kalori', 'Calories')}</Text>
                      <Text style={styles.clusterValue}>{route?.calories || `${Math.floor(elapsedTime * 0.1)} kcal`}</Text>
                    </View>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Gecen Sure', 'Elapsed Time')}</Text>
                      <Text style={styles.clusterValue}>{formatTime(elapsedTime)}</Text>
                    </View>
                    <View style={styles.clusterItem}>
                      <Text style={styles.clusterLabel}>{tx('Durum', 'Status')}</Text>
            ...
            <View style={styles.bottomActionsCompact}>
              ...
              <TouchableOpacity style={styles.endButtonCompact} onPress={handleFinishNavigation}>
                ...
                <Text style={styles.endButtonTextCompact}>{tx('Rotayı Bitir', 'Finish Route')}</Text>
              </TouchableOpacity>
            </View>
          ...
          {isSummaryCollapsed && (
            <View style={styles.summaryIndicatorWrap} {...indicatorPanResponder.panHandlers}>
              ...
              >
                ...
                <Text style={styles.summaryIndicatorText}>{tx('Ozeti Ac', 'Open Summary')}</Text>
              </TouchableOpacity>
            </View>
          )}
          </>
